...
echo     filevers=(1, 3, 9, 0), >> version_info_zh.txt
echo     prodvers=(1, 3, 9, 0), >> version_info_zh.txt
...
echo            StringStruct(u'FileDescription', u'JOVIAN指令通'), >> version_info_zh.txt
echo            StringStruct(u'FileVersion', u'%VERSION%'), >> version_info_zh.txt
echo            StringStruct(u'InternalName', u'JOVIAN指令通'), >> version_info_zh.txt
echo            StringStruct(u'LegalCopyright', u'Copyright (C) 2025 VALO360'), >> version_info_zh.txt
echo            StringStruct(u'OriginalFilename', u'JOVIAN指令通.exe'), >> version_info_zh.txt
echo            StringStruct(u'ProductName', u'JOVIAN指令通'), >> version_info_zh.txt
...
echo 開始打包程式...
python -m PyInstaller --onefile --clean --noconfirm --icon=app.ico -w --version-file=version_info_zh.txt --add-data "command.txt;." --add-data "user_guide.txt;." --add-data "setup.json;." --add-data "app.ico;." --name "JOVIAN指令通" main_utf8.py

echo.
if %ERRORLEVEL% EQU 0 (
    echo 打包成功! 輸出檔案位於 dist/JOVIAN指令通.exe
) else (
