...
class UIComponents(UIComponentsBase, UIComponentsInput, UIComponentsOutput, UIComponentsSettings):
    ...
    def init_settings_components(self):
        ...
        # 為所有主要按鈕加上 tooltip
        ToolTip(self.btn_clear, "清空回應區內容")
        ToolTip(self.btn_backup, "將回應內容備份成檔案")
        ToolTip(self.btn_guide, "顯示/隱藏使用說明")
        ToolTip(self.btn_refresh, "刷新可用的 COM 口列表")
        ToolTip(self.btn_content_font_plus, "增加內容字體大小")
        ToolTip(self.btn_content_font_minus, "減少內容字體大小")
        # 若有執行指令按鈕
        if hasattr(self, 'btn_exec'):
            ToolTip(self.btn_exec, "執行目前選擇的指令")
        # 若有其他按鈕可依需求補充
        # ...existing code...
    ...
    def init_tooltips(self):
        ...
        try:
            ...
            print("[DEBUG] 所有UI元件的AI tooltip已初始化完成")
            
        except Exception as e:
            print(f"[ERROR] 初始化tooltip時發生錯誤: {e}")
            import traceback
            traceback.print_exc()
    
    def _add_tooltip_to_widgets(self):
        """為所有UI元件添加AI生成的tooltip"""
        
        # 定義需要添加tooltip的元件列表
        widget_list = [
            # COM區域
            ('btn_refresh', 'refresh_button'),
            ('combobox_com', 'com_combobox'),
            ('label_com', 'com_label'),
            ('status_canvas', 'status_led'),
            
            # 指令區域
            ('combobox_cmd', 'command_combobox'),
            ('btn_execute', 'execute_button'),
            ('label_cmd', 'command_label'),
            ('section_description', 'section_description'),
            
            # Ping測試區域
            ('entry_ip', 'ip_entry'),
            ('btn_ping', 'ping_button'),
            ('btn_save_ip', 'save_ip_button'),
            ('btn_delete_ip', 'delete_ip_button'),
            ('btn_clear_ip', 'clear_ip_button'),
            ('label_ip', 'ip_label'),
            
            # 設定區域
            ('combobox_end', 'end_combobox'),
            ('btn_remove_end', 'remove_end_button'),
            ('entry_timeout', 'timeout_entry'),
            ('auto_exec_checkbox', 'auto_exec_checkbox'),
            ('label_end', 'end_label'),
            ('label_timeout', 'timeout_label'),
            
            # 字體控制區域
            ('btn_ui_font_plus', 'ui_font_plus_button'),
            ('btn_ui_font_minus', 'ui_font_minus_button'),
            ('ui_font_scale', 'ui_font_scale'),
            ('btn_content_font_plus', 'content_font_plus_button'),
            ('btn_content_font_minus', 'content_font_minus_button'),
            ('content_font_scale', 'content_font_scale'),
            ('label_ui_font', 'ui_font_label'),
            ('label_content_font', 'content_font_label'),
            
            # 功能按鈕區域
            ('btn_clear', 'clear_button'),
            ('btn_backup', 'backup_button'),
            ('btn_guide', 'guide_button'),
        ...
        # 為分類按鈕動態添加tooltip
        if hasattr(self, 'section_radiobuttons') and hasattr(self, 'sections'):
            for rb, sec in zip(self.section_radiobuttons, self.sections):
                self._add_ai_tooltip_to_widget(rb, f"section_{sec}")
    
    def _add_ai_tooltip_to_widget(self, widget, widget_name):
        """為單個元件添加AI生成的tooltip"""
        try:
            # 設定widget的名稱屬性，方便tooltip管理器識別
            widget._name = widget_name
            
            # 添加tooltip到元件（讓tooltip管理器自動從配置檔案獲取文字）
            self.tooltip_manager.add_tooltip(widget)
            
            print(f"[DEBUG] 已為 {widget_name} 添加tooltip")
            
        except Exception as e:
            print(f"[ERROR] 為 {widget_name} 添加tooltip時發生錯誤: {e}")