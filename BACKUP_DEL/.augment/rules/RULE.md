...
8. 禁止使用 HTML、Flask、Electron 等網頁前端技術；禁止使用不支援打包的框架（如 PyQt6 無 license）；禁止寫死資源絕對路徑；
9.超過300行代碼要切換新的PY
...
17.回答一律繁體中文
18.GitHub 上傳需等我要求時才做，並需同時上傳本地與遠端版本，若有提到版本時，需同時將版本訊息寫入到EXE的detail 和詳細資料，若程式有說明檔也寫入新增的功能
    打包後的 EXE 檔案版本資訊也要跟著修改
19.字體大小預設12，且需要新增一個+_的調整字體大小，此控制聯動界面和顯示的文字大小包含按鍵跟彈出視窗
20.一旦要求打包成EXE後－須要進行遠端git hub上傳 －同時進一版本
並且 把說明文補上
21.所有PY開頭都要有 詳細註解說明文 這個PY主要是做什麼用的 
22. 需要注意 新增代碼 不要在超過一千行的PY上增加，可以產生新的PY 或者在其他相關的PT名稱上增加