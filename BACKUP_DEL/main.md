...
## 檔案概述
這是指令通應用程式的主入口檔案，負責初始化應用程式、設置視窗屬性、管理日誌系統，並啟動圖形化使用者介面。
...
**說明：**
- 初始化錯誤處理器和配置管理器
- 將當前目錄和 ui_parts 目錄加入 Python 路徑，確保模組能正確導入
...
2. **日誌記錄**：詳細記錄程式執行過程，便於除錯
3. **資源管理**：程式關閉時自動清理資源
4. **配置管理**：支援動態載入和保存設定
5. **模組化設計**：將不同功能分離到不同模組中

## 依賴關係

- `core` 模組：提供錯誤處理、配置管理等功能
- `config_core`：配置檔案管理
- `ui_parts.ui_main`：主要UI介面
- `tkinter`：圖形化介面框架 