...
def save_setup(setup_data):
    ...
    try:
        ...
        # 處理 DUT_Control 和 Fixture_Control 中的 EndStrings
        for section in ['DUT_Control', 'Fixture_Control']:
            if section in setup_data and isinstance(setup_data[section], dict):
                # EndStrings 處理
        ...
        print(f"[DEBUG] 已保存完整的設定結構: {list(clean_setup.keys())}")
